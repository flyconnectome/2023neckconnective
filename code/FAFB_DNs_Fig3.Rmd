...
- plot the DN clusters by their primary brain neuorpil as in Fig. 3c
...
```{r, include=FALSE}
FAFB_DNs = data.table::fread(file = '/Users/tomke/Documents/dev/2023neckconnective/Supplemental_files/Supplemental_file5_FAFB_DNs.tsv')
...
DN_USsum_pie<- ggplot(DN_USsum, aes(x="", y=percent, fill=super_class)) +
  ...
  scale_fill_manual(values=DN_USsum$col) +  theme_minimal() +
  theme(axis.text.x=element_blank()) 
DN_USsum_pie
```
plot ouput partners by class as in Fig. 3a
```{r, include=FALSE}
...
DN_DSsum_pie<- ggplot(DN_DSsum, aes(x="", y=percent, fill=super_class)) +
  geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
  scale_fill_manual(values=DN_DSsum$col) +  theme_minimal() +
  theme(axis.text.x=element_blank()) 
DN_DSsum_pie
```

plot the DN clusters by their primary brain neuorpil
```{r, include=FALSE}
...
plot_neuropilgroup_sensorycluster2 = aggregate(plot_neuropilgroup_sensorycluster$number, by = list(plot_neuropilgroup_sensorycluster$cluster, plot_neuropilgroup_sensorycluster$neuropilgroup), FUN = sum) 
...
```{r, include=FALSE}
allsensory_ranking_type = fread(file = '/Users/tomke/Documents/dev/2023neckconnective/code/DN_ranking_df_783_sensory_combined_byDNtype.tsv')
